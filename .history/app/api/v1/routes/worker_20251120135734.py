...
async def registration(
    worker_data: WorkerRegistrationSchema, db: AsyncSession = Depends(get_db)
):
    """
    Create a new worker registration entry.
    """
    worker = await create_worker_service(workder_data, db)

    return custom_response(
        success=True,
        message=f"Worker {worker.name} created successfully.",
        data={"id": worker.id, "name": worker.name},
